...
interpreter commands = reverse (go 0 0 commands [])
  where
    go x y [] output = output
    go x y (cmd:cmds) output =
      case cmd of
        "up" -> go x (y + 1) cmds output
        "down" -> go x (y - 1) cmds output
        "left" -> go (x - 1) y cmds output
        "right" -> go (x + 1) y cmds output
        "printX" -> go x y cmds (show x : output)
        "printY" -> go x y cmds (show y : output)
        _ -> go x y cmds output
