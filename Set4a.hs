...
longest (x:xs) = foldr (\y acc -> if length y > length acc || (length y == length acc && head y < head acc) then y else acc) x xs
...
winner scores player1 player2 =
  case compare (score player1) (score player2) of
    LT -> player2
    EQ -> player1
    GT -> player1
  where
    defaultScore = 0
    score p = Map.findWithDefault defaultScore p scores
...
maxIndex arr = fst $ foldl1' maxIndexHelper $ assocs arr
  where maxIndexHelper x@(_, xval) y@(_, yval)
          | xval >= yval = x
          | otherwise    = y
